...
# URLs da API Node.js
url_quantidade_usuarios = "http://localhost:8079/apiDados/pessoasCertificado"
url_quantidade_certificado = "http://localhost:8079/apiDados/quantidadeCertificado"
...
try:
    ...
    if isinstance(total_pessoas, list) and len(total_pessoas) > 0:
        usuarios_com_certificado = int(total_pessoas[0].get('Quantidade_de_usuarios', 0))
    else:
    ...
    if isinstance(total_cetificados, list) and len(total_cetificados) > 0:
        total_usuarios = int(total_cetificados[0].get('Quantidade_de_certificados', 0))
    else:
